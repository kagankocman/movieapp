...
import { useGetTrendingMoviesQuery } from "../../../api/apiSlice.js";
import "./css/TopRated.css";
import { Spinner } from "reactstrap";
import { Tooltip } from "react-tooltip";
import { useSelector, useDispatch } from "react-redux";
import { addMovies } from "../../../redux/trendingMoviesSlice.js";
...
function TrendingMovies() {
    ...
    const { data, error, isFetching } = useGetTrendingMoviesQuery(currentPage);
    ...
    const handleItemsRendered = ({ visibleStartIndex, visibleStopIndex }) => {
      if (!isFetching && visibleStopIndex >= trendingMovies.length - 2) {
        setCurrentPage((prevPage) => prevPage + 1);
      }
      setIsAtStart(visibleStartIndex === 0);
      setIsAtEnd(visibleStopIndex >= trendingMovies.length - 1);
  
      if (visibleStopIndex >= trendingMovies.length - 2 && !isFetching) {
        setCurrentPage((prevPage) => prevPage + 1);
      }
    };
    ...
    return (
      <div className="movies-wrapper">
        <h2 className="cart-title">Trending Movies</h2>
        {!isAtStart && (
        ...
        >
          <FixedSizeList
            ...
            itemSize={190}
            itemCount={
              isFetching ? trendingMovies.length + 2 : trendingMovies.length
            }
            layout="horizontal"
